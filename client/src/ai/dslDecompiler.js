...
function decompileExpr(block, blockDefs) {
  if (!block) return 0;
  ...
  if (t === 'variables_get') {
    const v = block.fields?.VAR;
    return (typeof v === 'object' ? v.name : v) || 'x';
  }
  ...
  if (t === 'math_arithmetic') {
    ...
    if (block.inputs?.A?.block) dsl.a = decompileExpr(block.inputs.A.block, blockDefs);
    if (block.inputs?.B?.block) dsl.b = decompileExpr(block.inputs.B.block, blockDefs);
    return dsl;
  }
  ...
  if (t === 'math_modulo') {
    ...
    if (block.inputs?.DIVIDEND?.block) dsl.a = decompileExpr(block.inputs.DIVIDEND.block, blockDefs);
    if (block.inputs?.DIVISOR?.block) dsl.b = decompileExpr(block.inputs.DIVISOR.block, blockDefs);
    return dsl;
  }
  ...
  if (t === 'logic_negate') {
    ...
    if (block.inputs?.BOOL?.block) dsl.value = decompileExpr(block.inputs.BOOL.block, blockDefs);
    return dsl;
  }
  ...
  if (t === 'procedures_callreturn') {
    return decompileBlock(block, blockDefs);
  }
...
function decompileChain(block, blockDefs) {
  const chain = [];
  let current = block;
  while (current) {
    chain.push(decompileBlock(current, blockDefs));
...
function decompileBlock(block, blockDefs) {
  ...
  if (t === 'controls_whileUntil') {
    ...
    if (isForever) {
      ...
      if (block.inputs?.DO?.block) dsl.body = decompileChain(block.inputs.DO.block, blockDefs);
      return dsl;
    }

    const dsl = { type: 'controls_whileUntil', mode };
    if (boolBlock) dsl.condition = decompileExpr(boolBlock, blockDefs);
    if (block.inputs?.DO?.block) dsl.body = decompileChain(block.inputs.DO.block, blockDefs);
    return dsl;
  }
  ...
  if (t === 'controls_repeat_ext') {
    ...
    if (block.inputs?.TIMES?.block) dsl.times = decompileExpr(block.inputs.TIMES.block, blockDefs);
    if (block.inputs?.DO?.block) dsl.body = decompileChain(block.inputs.DO.block, blockDefs);
    return dsl;
  }
  ...
  if (t === 'controls_for') {
    const v = block.fields?.VAR;
    const dsl = { type: 'controls_for', var: (typeof v === 'object' ? v.name : v) || 'i' };
    if (block.inputs?.FROM?.block) dsl.from = decompileExpr(block.inputs.FROM.block, blockDefs);
    if (block.inputs?.TO?.block) dsl.to = decompileExpr(block.inputs.TO.block, blockDefs);
    if (block.inputs?.BY?.block) dsl.by = decompileExpr(block.inputs.BY.block, blockDefs);
    if (block.inputs?.DO?.block) dsl.body = decompileChain(block.inputs.DO.block, blockDefs);
    return dsl;
  }
  ...
  if (t === 'controls_forEach') {
    const v = block.fields?.VAR;
    const dsl = { type: 'controls_forEach', var: (typeof v === 'object' ? v.name : v) || 'item' };
    if (block.inputs?.LIST?.block) dsl.list = decompileExpr(block.inputs.LIST.block, blockDefs);
    if (block.inputs?.DO?.block) dsl.body = decompileChain(block.inputs.DO.block, blockDefs);
    return dsl;
  }
  ...
  if (t === 'controls_if') {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (condBlock) dsl[`if${i}`] = decompileExpr(condBlock, blockDefs);
      if (bodyBlock) dsl[`do${i}`] = decompileChain(bodyBlock, blockDefs);
    }
    if (block.inputs?.ELSE?.block) dsl.else = decompileChain(block.inputs.ELSE.block, blockDefs);
    return dsl;
  }
  ...
  if (t === 'variables_set') {
    const v = block.fields?.VAR;
    const dsl = { type: 'variables_set', var: (typeof v === 'object' ? v.name : v) || 'x' };
    if (block.inputs?.VALUE?.block) dsl.value = decompileExpr(block.inputs.VALUE.block, blockDefs);
    return dsl;
  }
  ...
  if (t === 'procedures_defnoreturn') {
    ...
    if (block.inputs?.STACK?.block) dsl.body = decompileChain(block.inputs.STACK.block, blockDefs);
    return dsl;
  }
  ...
  if (t === 'procedures_defreturn') {
    ...
    if (block.extraState?.params) dsl.args = block.extraState.params.map(p => p.name || p);
    if (block.inputs?.STACK?.block) dsl.body = decompileChain(block.inputs.STACK.block, blockDefs);
    if (block.inputs?.RETURN?.block) dsl.return_value = decompileExpr(block.inputs.RETURN.block, blockDefs);
    return dsl;
  }
  ...
  if (t === 'procedures_callnoreturn' || t === 'procedures_callreturn') {
    ...
    if (block.extraState?.params?.length) {
      dsl.args = block.extraState.params.map((_, i) => {
        ...
        return inp ? decompileExpr(inp, blockDefs) : 0;
      });
    }
    return dsl;
  }
  ...
  if (def) {
    ...
    if (block.fields) {
      for (const [key, value] of Object.entries(block.fields)) {
        if (key === 'VAR') {
          dsl.var = (typeof value === 'object' ? value.name : value) || '';
    ...
    if (block.inputs) {
      for (const [key, inp] of Object.entries(block.inputs)) {
        ...
        if (meta?.check === 'Pin' && inp.block.type === 'esp32_gpio_pin') {
          ...
          dsl[key.toLowerCase()] = decompileExpr(inp.block, blockDefs);
        }
      }
    }
...
export function decompileDSL(workspaceJson) {
  ...
  return workspaceJson.blocks.blocks.map(topBlock => decompileChain(topBlock, blockDefs));
}
