...
### 2. Rosbridge Server
- **Role**: WebSocket Bridge for ROS.
- **Technology**: `rosbridge_suite` (ROS 2 Jazzy).
- **Networking**:
    - **Port**: `9090` (Mapped to Host).
    - **Internal**: Connects to `ros_net`.
- **Communication**:
    - **Input**: JSON messages from Client (WebSocket).
    - **Output**: Native ROS 2 messages (DDS) on topics like `/cmd_vel`.
    - **Discovery**: Automatically discovers other ROS nodes on the `ros_net` network using standard ROS 2 discovery (multicast).

### 3. Robot (Simulated Controller)
- **Role**: Robot Logic / Controller.
- **Technology**: ROS 2 Jazzy (C++/Python nodes).
- **Networking**:
    - **Internal**: Connects to `ros_net`.
- **Communication**:
    - **Input**: Subscribes to `/cmd_vel` (from Rosbridge or Simulator).
    - **Output**: Publishes telemetry (e.g., `/odom`).
    - **Note**: In this simulation setup, the "Robot" container acts as a placeholder or high-level controller, while the physical simulation happens in Gazebo.

### 4. Simulator (Gazebo)
- **Role**: Physics Simulation and Visualization.
